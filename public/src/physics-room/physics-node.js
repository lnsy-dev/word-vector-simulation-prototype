...
class PhysicsNode extends DataroomElement {
  ...
  calculateSimilarity(otherVector) {
    const dotProduct = this.vector.reduce(
      (acc, val, i) => acc + val * otherVector[i],
      0,
    );
    const normA = Math.sqrt(
      this.vector.reduce((acc, val) => acc + val * val, 0),
    );
    const normB = Math.sqrt(
      otherVector.reduce((acc, val) => acc + val * val, 0),
    );
  ...
  applyForce(force, similarity) {
    if (this.rigidBody) {
      this.rigidBody.applyImpulse(
        {
          x: force[0] * similarity,
          y: force[1] * similarity,
          z: force[2] * similarity,
        },
        true,
      );
    }
